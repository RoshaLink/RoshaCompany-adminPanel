  ...
  return (
    <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 mb-6">
      ...
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2 sm:gap-2.5 w-full md:w-auto">
        {/* Status Filter */}
        <div className="relative col-span-1 sm:flex-none">
          <select
            value={filters.status}
            onChange={handleStatusChange}
            className="w-full pl-3 rtl:pl-8 rtl:pr-3 pr-8 py-2 sm:py-2.5 rounded-xl bg-white/80 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-sky-500 text-slate-700 dark:text-slate-200 shadow-sm appearance-none cursor-pointer"
          >
          ...
          <Filter className="w-3.5 h-3.5 absolute right-2.5 rtl:right-auto rtl:left-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
        </div>
        ...
        {hasActiveFilters && (
          <button
            ...
            className="col-span-1 sm:col-auto px-3 py-2 sm:py-2.5 rounded-xl text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800/60 hover:bg-rose-100 transition-colors cursor-pointer text-center"
          >
            {t('common.reset')}
          </button>
        )}
