...
import { STATUS_CONFIG, getLocalizedStatus } from './LeadStatusBadge';
...
Message:
${selectedLead.message || '(No message)'}${selectedLead.notes ? `\n\nInternal Notes:\n${selectedLead.notes}` : ''}`;
        ...
        <div className="p-3.5 sm:p-6 border-b border-slate-100 dark:border-slate-800/80 flex items-start justify-between gap-2.5 sm:gap-3 bg-gradient-to-b from-slate-50/50 dark:from-slate-800/20 to-transparent">
          <div className="space-y-1 flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 flex items-center gap-1.5 shrink-0">
                <Globe2 className="w-3.5 h-3.5 text-sky-500" />
                <span>{selectedLead.lang?.toUpperCase() || 'SV'}</span>
              </span>
          ...
          <div className="space-y-2 pt-1">
            ...
            {isEditing ? (
              <textarea
                ...
                rows={3}
                placeholder={t('leadDetail.adminNotesPlaceholder')}
                className="w-full p-3.5 sm:p-4 rounded-2xl bg-amber-50/30 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/60 focus:outline-none focus:ring-2 focus:ring-amber-500 text-slate-900 dark:text-white leading-relaxed text-sm resize-y placeholder:text-slate-400"
              />
            ) : (
              <div className="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-br from-amber-50/40 via-slate-50/60 to-slate-50/60 dark:from-amber-950/15 dark:via-slate-800/60 dark:to-slate-800/60 border border-amber-200/60 dark:border-amber-800/40 text-slate-800 dark:text-slate-200 whitespace-pre-wrap leading-relaxed text-sm">
                {selectedLead.notes ? (
                  <span>{selectedLead.notes}</span>
                ) : (
                  <div className="flex items-center justify-between gap-2">
                    <span className="italic text-slate-400 text-xs">{t('leadDetail.noAdminNotes')}</span>
                    <button
                      type="button"
                      onClick={() => setIsEditing(true)}
