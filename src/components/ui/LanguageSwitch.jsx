  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl bg-slate-100/90 dark:bg-slate-800/90 border border-slate-200/90 dark:border-slate-700/80 hover:border-sky-400 dark:hover:border-sky-500 text-slate-700 dark:text-slate-200 text-xs font-semibold shadow-sm transition-all hover:scale-105 active:scale-95 cursor-pointer focus:outline-none focus:ring-2 focus:ring-sky-500/40"
        title="Switch Language / تغییر زبان"
      >
        <span className="text-sm leading-none">{currentLang.flag}</span>
        {!compact && (
          <span className="font-mono uppercase tracking-wider hidden xs:inline text-[11px] sm:text-xs">{currentLang.code}</span>
        )}
        <ChevronDown
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 rtl:right-auto rtl:left-0 mt-2 w-36 sm:w-44 rounded-2xl bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 shadow-2xl backdrop-blur-xl p-1.5 z-50 animate-scaleUp">
