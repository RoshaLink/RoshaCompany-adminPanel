...
import { getLocalizedSource } from './RecentLeadsCard';
...
export const SourceDistribution = () => {
  ...
  const sourceKeys = ['connect-with-us', 'contact', 'get-started', 'chat'];
    ...
    <GlassCard className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-headline font-bold text-sm text-slate-900 dark:text-white">
            {t('dashboard.funnelsTitle')}
          </h3>
        </div>
        <span className="text-xs text-slate-400 font-mono">
          {t('dashboard.funnelsSubtitle')}
        </span>
      </div>
      ...
      <div className="space-y-4">
        {sourceKeys.map((key) => {
          ...
          const label = getLocalizedSource(key, t);
            ...
            >
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-2 font-mono">
                  ...
                  <ArrowUpRight className="w-3 h-3 opacity-0 group-hover:opacity-100 text-sky-500 transition-opacity rtl:rotate-90" />
                </div>
              </div>
